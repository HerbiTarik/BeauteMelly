...
if (isset($_POST['submit'])) {
	...
	if ($password == $cpassword) {
		...
		if (!$result->num_rows > 0) {
			...
			$result = mysqli_query($conn, $sql);
			if ($result) {
				echo "<script>alert('Utilisateur ajouté avec succès! Veuillez vous connecter.')</script>";
				$username = "";
				$email = "";
				$_POST['password'] = "";
				$_POST['cpassword'] = "";
			} else {
				echo "<script>alert('Il y a une erreur quelque part.')</script>";
			}
		} else {
			echo "<script>alert('Cette adresse existe déjà.')</script>";
		}
		
	} else {
		echo "<script>alert('Les mots de passe sont incorrects.')</script>";
	}
}
...
<nav class="navbarre">
       ...
       <div class="connexion">
		<form action="" method="POST" class="login-email">
			...
			<p class="login-register-text">Avez-vous un compte ? <a href="connexion.php">connexion</a>.</p>
		</form>
	</div>
</div>
